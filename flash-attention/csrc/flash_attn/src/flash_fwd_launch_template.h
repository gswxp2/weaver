...
DEFINE_FLASH_FORWARD_KERNEL(flash_fwd_splitkv_kernel, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV) {
    ...
    if (params.check_meta_ptr!=nullptr) {
        int* void_ptr = (int*)params.check_meta_ptr;
        // check todo > finished
        int todo = void_ptr[0];
        int finished = void_ptr[1];
        // assert (todo >= finished);
        if (todo == finished) {
            // if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && threadIdx.x == 0){
            //     // printf("aatodo is %d, finished is %d\n", void_ptr[0], void_ptr[1]);
            //     // printf("block xyz is %d %d %d\n", blockIdx.x, blockIdx.y, blockIdx.z);
            //     // printf("todo is %d, finished is %d\n", todo, finished);
            // }
            return;
        }
    }
    ...
    if (params.check_meta_ptr!=nullptr) {
        __syncthreads();
        // printf("the kernel finished! 123\n");
        int* void_ptr = (int*)params.check_meta_ptr;
        // check todo > finished
        if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && threadIdx.x == 0){
            void_ptr[2] = void_ptr[0];
        }
        // printf("the kernel finished!\n");
    }
}
