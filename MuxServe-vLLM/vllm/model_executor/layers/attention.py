...
class PagedAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.fake_block_tables = None
    def init_fake_block_tables(self, key_cache, input_metadata):
        is_muxserve = input_metadata.block_tables.ndim == 3
        assert(type(key_cache) == torch.Tensor)
        key_cache = key_cache.view(-1, 32 , self.num_kv_heads, 128)
        total_blocks = key_cache.shape[0]
        # max 256 sequence and 64 blocks
        # we generate a 256 * 64 random int choice from 0 to total_blocks
        # then we convert it to a tensor of shape 256, 64
        import numpy as np
        np.random.seed(0)
        self.fake_block_tables = torch.tensor(np.random.choice(total_blocks, 512*64), dtype=torch.int32, device=key_cache.device).view(512, 64)
    ...
    def forward(
        ...
        """
        if (self.fake_block_tables is None):
            self.init_fake_block_tables(key_cache, input_metadata)
        ...
        if input_metadata.num_generation_tokens > 0:
            ...
            sequence = input_metadata.block_tables.shape[1]
            max_blocks = input_metadata.block_tables.shape[-1]
            flash_attn_with_kvcache(
                q = query[num_prompt_tokens:num_valid_tokens].unsqueeze(1),
                k_cache = key_cache.view(-1, 32 , self.num_kv_heads, 128),
                v_cache = value_cache.view(-1, 32 , self.num_kv_heads, 128),
                check_meta = None,
                block_table = self.fake_block_tables[:sequence, :max_blocks],
            ...
            # print(input_metadata.block_tables.shape)
