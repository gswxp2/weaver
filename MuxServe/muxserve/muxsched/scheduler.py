...
class MuxScheduler:
    ...
    def serve_models(self):
        ...
        for job_config in self.muxserve_config.job_configs:
            ...
            self.model_to_blocks_per_token[
                model_name] = model_config.num_key_value_heads// tensor_parallel_size * num_hidden_layers
    ...
    def adapt_max_num_seqs(self):
        ...
        for model_name in self._served_models:
            ...
            print("avg_blocks_per_seq", avg_blocks_per_seq)
            print("avg seq len", avg_seq_len)
            print("model to blocks per token", self.model_to_blocks_per_token)
    ...
    def init_max_num_seqs(self, fixed=False) -> int:
        ...
        # sharegpt default
        avg_seq_len_dict = {
            model_name: int(os.environ.get("AVG_SEQ_LEN", 900))
            for model_name in self._served_models
        }
    ...
    def warn_log(self, last_sched_tick, last_warn_tick, status, model,
                 prefill):
        if self.get_tick() - last_sched_tick > 5 and self.get_tick(
        ) - last_warn_tick > 5:
            ...
            return True
        return False
    def mark_prefilled(self):
       for i in range(len(self._served_models)):
            model = self._served_models[i]
            queue = self.waiting[model]
            while queue:
                req = queue.pop(0)
                req_id = req.idx
                req = self.workload.requests[req_id]
                req.prefill_end_time = self.get_tick()
                req.output_tokens = [200]
    ...
    def log_stats(self):
        ...
        for prefix in ["nosched", "sched"]:
            add_sched = (prefix == "nosched")
            for model_name, requests in model_to_requests.items():
                # filter the first 20% and last 20% requests
                requests = requests[int(len(requests) * 0.2):int(
                    len(requests) * 0.8)]
