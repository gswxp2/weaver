...
DEFAULT_WARMUP = 10
DEFAULT_DATASET_PATH = "/home/weaverae/weaverae/ShareGPT_V3_unfiltered_cleaned_split.json"
DEFAULT_TOKENIZER_PATH = "meta-llama/Meta-Llama-3-8B"
eps = 1e-6
...
def sample_requests(
    ...
    print(num_requests)
    if "azure" in dataset_path or "burst" in dataset_path:
        ...
        for req in sampled_requests:
            ...
            all_res.append(([200 for _ in range(input_len)], input_len, output_len))
        return all_res
    assert False
    if dataset_path == "random":
        piece = [[200 for _ in range(input_len)] , input_len, output_len]
        return [piece for _ in range(num_requests)]
...
def sample_request_datas(
    ...
    seqlen_distribution: Optional[Tuple[int, int]] = None,
    tokenized_cache_path: str = "/home/weaverae/weaverae/ShareGPT_V3_llama_tokenized.cache",
):
    # sample requests to fill the workload
    if tokenizer is None and (tokenized_cache_path is None or not os.path.exists(tokenized_cache_path)):
        tokenizer = get_tokenizer(DEFAULT_TOKENIZER_PATH)
        
    print(tokenized_cache_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--runtime", type=int, default=90, help="the run time of the workload")
    parser.add_argument("--input", type=int, default=1024, help="the input length of the workload")
    parser.add_argument("--output", type=int, default=128, help="the output length of the workload")
    global args
    ...
    if workload_info_from_yaml:
        ...
        num_requests = max(rate_dist) * args.runtime
        ...
        num_requests_dist = []
        dispatch_duration = args.runtime
        ...
        # output_file = f"/home/weaverae/weaverae/MuxServe/examples/workloads/sharegpt_uneven_n{len(rate_dist)}_max{max_rate}.json"
        # output_file = f"/home/weaverae/weaverae/MuxServe/examples/workloads/sharegpt_fairness_n{len(rate_dist)}_rate{rate_str}.json"
        output_file = f"/home/weaverae/weaverae/MuxServe/examples/workloads/fairness_difflen_n{len(rate_dist)}_rate{rate_str}.json"
        generate_workload(workload_infos,
                          output_file,
                          num_requests=num_requests_dist,
                          sampled_requests=sampled_requests,
        ...
        for rate in [11]:
            ...
            # output_file = f"/home/weaverae/weaverae/MuxServe/examples/workloads/sharegpt_n{num_models}_rate{rate}.json"
            # output_file = f"/mnt/afs/lurunyu/projects/MuxServe/examples/workloads/sharegpt_n{num_models}_rate{rate}.json"
            # output_file = f"/home/weaverae/weaverae/MuxServe/examples/workloads/sharegpt_fairness_n{num_models}_rate{rate}.json"
            # output_file = f"/home/weaverae/weaverae/MuxServe/examples/workloads/sharegpt_n{num_models}_p256_g128.json"
            output_file = args.output_file
