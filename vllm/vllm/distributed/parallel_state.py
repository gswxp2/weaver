...
class GroupCoordinator:
    ...
    def __init__(
        ...
        dist_group_ranks: List[List[int]],
        local_rank: int,
        ...
        # world = int(os.environ.get("GLOBAL_WORLD"))
        try:
            global_idx = int(os.environ.get("GLOBAL_IDX",0))
            group_ranks = [[r + global_idx for r in ranks] for ranks in group_ranks]
        except:
            group_ranks = dist_group_ranks
        self.origin_rank = local_rank
        local_rank += global_idx
        ...
        self.cpu_group = None
        self.last_work = None
        self.size_tensor = torch.empty(1, dtype=torch.long, device="cpu")
        ...
        if current_platform.is_cuda_alike():
            self.device = torch.device(f"cuda:{self.origin_rank}")
        else:
    ...
    def recv_object_async(self, src: int) -> Any:
        """Receive the input object list from the source rank."""
        """NOTE: `src` is the local rank of the source rank."""
        ...
        if self.last_work is None:
            ...
            req = torch.distributed.irecv(
                self.size_tensor, src=self.ranks[src], group=self.cpu_group
            )
            ...
            if self.last_work.is_alive():
                return None
            rank_size = self.size_tensor.item()
            # Tensor to receive serialized objects into.
            object_tensor = torch.empty(rank_size, dtype=torch.uint8, device="cpu")
            self.last_work = torch.distributed.recv(
    ...
    def send(
        self, tensor: torch.Tensor, dst: Optional[int] = None, stream=None
    ) -> None:
        ...
        if pynccl_comm is not None and not pynccl_comm.disabled:
            pynccl_comm.send(tensor, dst, stream)
        else:
    ...
    def recv(
        ...
        pynccl_comm.disabled = False
        if pynccl_comm is not None and not pynccl_comm.disabled:
            pynccl_comm.recv(tensor, src, stream)
        else:
...
_DISPAIR: Optional[GroupCoordinator] = None
ATTENTION_META = None
...
def init_dis_pair_group(local_rank: int) :
    global _DISPAIR
    _DISPAIR = GroupCoordinator(
        group_ranks=None,
        dist_group_ranks=[range(torch.distributed.get_world_size())],
    ...
    global ATTENTION_META
    from vllm.distributed.device_communicators.attn_check_meta import create_attn_buffer
    ATTENTION_META = create_attn_buffer(12, _DISPAIR.cpu_group)
    if get_role() == "SENDER":
        torch.distributed.barrier()
    assert _DISPAIR is not None, ("dispair group is not initialized")


def get_attn_meta() -> torch.Tensor:
    assert ATTENTION_META is not None, ("attention meta is not initialized")
...
def get_dis_group() -> GroupCoordinator:
    global _DISPAIR
    assert _DISPAIR is not None, ("dispair group is not initialized")
    return _DISPAIR
...
def initialize_model_parallel(
    ...
    world_size: int = _WORLD.world_size
