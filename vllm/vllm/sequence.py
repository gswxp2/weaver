...
class SequenceData(msgspec.Struct,
    ...
    def update_num_computed_tokens(self, num_new_computed_tokens: int):
        ...
        if self.get_num_uncomputed_tokens() == 0:
            # print("shift from prefill to decode")
...
class Sequence:
    ...
    def append_token_only(self, token_id: int) -> None:
        self.output_logprobs.append(None)
...
THRESHOLD = float(os.environ.get("OFFLOAD_PERCENT", 45)) / 100
print("The offload threshold is set to", THRESHOLD)
print("The offload threshold is set to", THRESHOLD)
print("The offload threshold is set to", THRESHOLD)
class SequenceGroup:
    ...
    def __init__(
        ...
        self.should_offload = random.random() < THRESHOLD
        # self.should_offload = False
        from vllm.distributed import offload_enabled, get_role

        if (not offload_enabled()) or get_role() == "RECEIVER":
            self.should_offload = False
