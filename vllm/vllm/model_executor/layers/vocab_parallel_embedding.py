...
from vllm.worker.daemon.daemon_work import step,print_step_count,chunk_enabled,embedding_splits
...
from vllm.worker.daemon.daemon_work import step,print_step_count,chunk_enabled


class ShardedUnquantizedEmbeddingMethod(UnquantizedEmbeddingMethod):
    ...
    def apply(self,
              ...
              x: torch.Tensor,
              bias: Optional[torch.Tensor] = None) -> torch.Tensor:
        if chunk_enabled and get_role() == "RECEIVER" and offload_enabled():
            if self.first:
                self.output = torch.zeros(1024, layer.weight.size(0), device=x.device, dtype=torch.bfloat16)
                self.embedding_dimension = layer.weight.size(0) 
                assert self.embedding_dimension % embedding_splits == 0
                self.first = False
            step()
            for i in range(embedding_splits):
                start, end = self.embedding_dimension//embedding_splits * i, self.embedding_dimension//embedding_splits * (i+1)
                # output = F.linear(x.float(), layer.weight[start:end].float(), bias)
                F.linear(x, layer.weight[start:end], bias, out=self.output[:x.shape[0], start:end])
                step()
            return self.output[:x.shape[0]]
        else:
            return F.linear(x, layer.weight, bias)
...
class VocabParallelEmbedding(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if linear_method is None:
            linear_method = ShardedUnquantizedEmbeddingMethod()
