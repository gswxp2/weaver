...
def _greedy_sample(
    ...
    if get_role() == "RECEIVER":
        global samples_cpu,event
        if samples_cpu is None:
            samples_cpu = torch.zeros(256, device="cpu", dtype=torch.int32, pin_memory=True)
            event = torch.cuda.Event(enable_timing=True)
        # event.record()
        # samples_cpu = torch.empty_like(samples, device="cpu") 
        samples_cpu[0:samples.shape[0]].copy_(samples, non_blocking=True)
        samples_lst = samples_cpu[0:samples.shape[0]].tolist()
