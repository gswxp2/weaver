...
class QWenAttention(nn.Module):
    ...
    def __init__(
        ...
        self.recv_attention = torch.zeros(8192, 5120, dtype=torch.bfloat16).cuda()
    ...
    def forward(
        ...
        offload_position = attn_metadata.offload_position
        step()
        qkv, _ = self.qkv_proj(hidden_states)
        step()

        # assert qkv_offload.contiguous() == True
        if attn_metadata.should_offload :
            qkv_offload = qkv[offload_position:]
            offload_len = hidden_states.shape[0] - offload_position
            # global offload_stream
            # offload_stream.wait_stream(torch.cuda.default_stream())
            # torch.cuda.set_stream(offload_stream)
            # print(qkv_offload.shape)
            # q = qkv_offload[:, :self.q_size]
            # k = qkv_offload[:, self.q_size:self.q_size + self.kv_size]
            # v = qkv_offload[:, self.q_size + self.kv_size:]
            # print(v.mean())
            if offload_len > 0:
                get_attn_meta()[2][:offload_len].copy_(qkv_offload)
                get_attn_meta()[1][0]+=1
            qkv = qkv[:offload_position]

            # print(q[offload_position:].mean())
            # print(k[offload_position:].mean())
            # print(v[offload_position:].mean())
            # nccl_group_start()
            # sr_tensor(q[offload_position:].contiguous(), 0, 1)
            # sr_tensor(k[offload_position:].contiguous(), 0, 1)
            # sr_tensor(v[offload_position:].contiguous(), 0, 1)
            # nccl_group_end()
            # sr_tensor(qkv_offload, 0, 1)
            # torch.cuda.set_stream(torch.cuda.default_stream())
        else:
            offload_len = qkv.shape[0]
        q, k, v = qkv.split([self.q_size, self.kv_size, self.kv_size], dim=-1)
        # global exchange_meta
        # we put half of the sequence into the exchange_meta tensor
        # sendrecv_tensor(exchange_meta, 0, 1)
        if offload_position > 0:
            q, k = self.rotary_emb(
                positions[:offload_position], q, k
            )
            ...
            attn_output = None
        step()
        if attn_metadata.should_offload and offload_len > 0:
            # sr_tensor(self.recv_attention[:offload_len], 1, 0)
            torch.ops._C.poll_offload(get_attn_meta()[1])
            if attn_output is None:
                attn_output = self.recv_attention[:offload_len]
            else:
                attn_output = torch.cat(
                    [attn_output, self.recv_attention[:offload_len]], dim=0
                )
            # sr_tensor(recv_attention, 1, 0)
            # print(torch.norm(recv_attention - attn_output[offload_position:]))
            # attn_output = torch.cat([attn_output, recv_attention], dim=0)

        # attn_output1 = self.recv_attention[: q.shape[0]]
        # assert attn_output1.shape == attn_output.shape
