...
from vllm.distributed import get_role, offload_enabled
from vllm.worker.daemon.daemon_work import (
    init_daemon_worker,
    print_daemon_worker,
    DAEMON_WORKER,
)
...
class LLMEngine:
    ...
    def __init__(
        ...
        if get_role() == "RECEIVER" and offload_enabled():
            init_daemon_worker(
                SelfAttnBlockSpaceManager.get_instance(),
                self.model_executor.driver_worker.kv_cache[0],
            )
    ...
    def _process_model_outputs(self,
        ...
        for i in indices:
            ...
            if not is_async:
                if self.scheduler_config.is_multi_step:
                    ...
                    original_stage = seq_group.seqs[0].data._stage
                    seq_group.update_num_computed_tokens(
                        seq_group_meta.token_chunk_size or 0)
                    new_stage = seq_group.seqs[0].data._stage
                    if original_stage != new_stage:
                        self.last_prefill = time.time()
                        self.last_offload_time = time.time()
                        # we should offload the kv cache to the neighbor
                        # if seq_group.should_offload:
                        #     to_offload_seqs.append(seq_group)
                        #     to_offload_seq_metas.append(seq_group_meta)
            if outputs:
                for o in outputs:
        ...
        if len(to_offload_seqs) > 0 and offload_enabled():
            self.offload_sequence(to_offload_seqs, to_offload_seq_metas)
    ...
    def _has_remaining_steps(
        ...
        if any([
            ...
            for seq_group in seq_group_metadata_list:
                logger.info(
                    f"SeqGroup {seq_group.request_id} has remaining steps "
                    f"{seq_group.state.remaining_steps}")
    ...
    def offload_sequence(
        self, seq: List[SequenceGroup], seq_meta: List[SequenceGroupMetadata]
    ) -> None:
        assert False
        """Offloads a sequence to the model executor."""
        print("Offloading sequence to model executor", len(seq))
        self.model_executor.offload_sequence(seq, seq_meta)
        print("Offload finish")
        # we remove all the kv cache of the sequence
        for seq_group in seq:
            self.scheduler[0].block_manager.free(seq_group.seqs[0])
        self.last_offload_time = time.time()
