...
from vllm.worker.daemon.daemon_work import step
...
class WorkerBase(ABC):
    ...
    def __init__(
        ...
        self.meta_cache["seq_ids"] = []
        self.meta_cache["seq_lengths"] = []
        self.meta_cache["seq_count"] = 0
...
class LocalOrDistributedWorkerBase(WorkerBase):
    ...
    def _get_driver_input_and_broadcast(
        ...
        if offload_enabled():
            self.prepare_offload_input(execute_model_req)
        ...
        if self.do_metadata_broadcast:
            ...
            # sr_tensor_dict(model_input, src=0)
    ...
    def execute_model(
        ...
        # print(model_input.input_tokens.shape)
    ...
    def prepare_offload_input(
        self, execute_model_req: ExecuteModelRequest
    ) -> Dict[str, torch.Tensor]:
        """
        Prepare the input for offloading to other workers.
        """
        seq_list = execute_model_req.seq_group_metadata_list
        seq_count = 0
        self.meta_cache["seq_ids"].clear()
        self.meta_cache["seq_lengths"].clear()
        self.meta_cache["seq_count"] = 0
        for seq_group_metadata in seq_list:
            if seq_group_metadata.should_offload and not seq_group_metadata.is_prompt:
                # print(
                #     seq_group_metadata.seq_data[
                #         int(seq_group_metadata.request_id)
                #     ].get_len()
                # )
                self.meta_cache["seq_ids"].append(int(seq_group_metadata.request_id))
                self.meta_cache["seq_lengths"].append(
                    seq_group_metadata.seq_data[
                        int(seq_group_metadata.request_id)
                    ].get_len()
                )
                seq_count += 1
        self.meta_cache["seq_count"] = seq_count
        if seq_count > 0:
            sr_obj_async(self.meta_cache, src=0, dst=1)
