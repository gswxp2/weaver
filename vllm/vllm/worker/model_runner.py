...
class ModelInputForGPUBuilder(ModelRunnerInputBuilderBase[ModelInputForGPU]):
    ...
    def _compute_multi_modal_input(self, inter_data: InterDataForSeqGroup,
        ...
        if len(inter_data.input_tokens) == 0:
            print(inter_data, seq_group_metadata)
            return
    ...
    def add_seq_group(self, seq_group_metadata: SequenceGroupMetadata):
        ...
        inter_data = self.init_cached_inter_data(
            ...
            seq_ids=seq_ids,
            is_prompt=is_prompt,
            should_offload=seq_group_metadata.should_offload and not is_prompt,
...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        if prefill_meta is None and False and decode_meta.use_cuda_graph :
        ...
        nvtx.pop_range()
        print(f"ModelRunner: {self.start_event.elapsed_time(self.end_event)}",flush=True)
        if (self.observability_config is not None
                and self.observability_config.collect_model_forward_time
