...
from vllm.sequence import (Sequence, SequenceData, SequenceGroup,
                           ...
                           SequenceStatus)
from vllm.distributed.communication_op import sr_obj, sr_obj_async
from vllm.distributed import offload_enabled, get_role
...
class Scheduler:
    ...
    def _schedule_running(
        ...
        offload_scheduled_decode_seq_groups = []
        offload_decode_seq_groups = []
        running_queue = self.running
        assert len(self._async_stopped) == 0
        for req in running_queue:
            if not req.is_prefill() and req.should_offload:
                r = self._scheduled_seq_group_cache[self.cache_id].get_object()
                r.seq_group = req
                r.token_chunk_size = 1
                offload_scheduled_decode_seq_groups.append(r)
                offload_decode_seq_groups.append(req)

        while running_queue:
            seq_group = running_queue[0]
            if not seq_group.is_prefill() and seq_group.should_offload:
            ...
            while (not seq_group.should_offload )and (not self._can_append_slots(seq_group, enable_chunking)):
                print(self.block_manager.get_num_free_gpu_blocks())
                print(self.block_manager.block_tables)
                for key in self.block_manager.block_tables:
                    print(key, len(self.block_manager.block_tables[key].blocks))
                ...
                if not seq_group.should_offload:
        ...
        # we sort the decode_seq_groups by whether need to be offloaded
        decode_seq_groups.extend(offload_scheduled_decode_seq_groups)
        ret.decode_seq_groups_list.extend(offload_decode_seq_groups)
        return ret
    ...
    def _mark_prefilled(
        ...
        while self._passed_delay(time.time()) and waiting_queue:
            ...
            num_prompt_tokens = waiting_seqs[0].get_len()
            num_new_tokens = num_prompt_tokens

            num_lookahead_slots: int = 0
            if self.scheduler_config.is_multi_step and False:
                num_lookahead_slots = self._get_num_lookahead_slots(
                    True, enable_chunking)
            ...
            can_allocate = self.block_manager.can_allocate(
                seq_group, num_lookahead_slots=num_lookahead_slots)
            if can_allocate == AllocStatus.LATER:
                print("You have too much kv cache blocks!:",self.block_manager.get_num_free_cpu_blocks())
                break
            elif can_allocate == AllocStatus.NEVER:
                logger.warning(
                    "Input prompt (%d tokens) + lookahead slots (%d) is "
            ...
            # print("mark prefilled", seq_group.request_id, seq_group.get_num_uncomputed_tokens())
            # seq_group.init_multi_step_from_lookahead_slots(
            #     num_lookahead_slots,
            #     num_scheduler_steps=self.scheduler_config.
            #     num_scheduler_steps,
            ...
            seq.append_token_only(200)
            self.running.append(seq_group)
            
            self.offload_request = {}
            self.offload_request["type"] = "migrate"
            self.offload_request["seq_nums"] = 0
            self.offload_request["seq_ids"] = []
            self.offload_request["seq_lengths"] = []
            def offload_sequence(self, seqgroup):
                # step 1: send a dict to the receiver including
                # seq_id, seq_length
                self.offload_request["seq_ids"].clear()
                self.offload_request["seq_lengths"].clear()
                self.offload_request["seq_ids"].append(int(seqgroup.request_id))
                self.offload_request["seq_lengths"].append(seqgroup.seqs[0].get_prompt_len())
                self.offload_request["seq_nums"] = 1
                sr_obj_async(self.offload_request, 0, 1)
            # print(self.block_manager.get_block_table(seq_group.seqs[0]))
            if get_role() == "SENDER" and offload_enabled() and seq_group.should_offload:
                offload_sequence(self, seq_group)
                # print("block manager free", seq_group.request_id)
                self.block_manager.free(seq_group.seqs[0])
    ...
    def schedule(
        ...
        for i, scheduled_seq_group in enumerate(
            ...
            for seq in seq_group.get_seqs(status=SequenceStatus.RUNNING):
                ...
                if seq_group.should_offload and not seq_group.is_prefill():
            ...
            if is_first_prefill or not self.scheduler_config.send_delta_data:
                seq_group_metadata = SequenceGroupMetadata(
                    ...
                    should_offload=seq_group.should_offload,
                    do_sample=do_sample,
    ...
    def free_finished_seq_groups(self) -> None:
        ...
        self.free_requests["seq_ids"].clear()
        for seq_group in self.running:
            if seq_group.is_finished() and seq_group.should_offload:
                self.free_requests["seq_ids"].append(int(seq_group.request_id))
            self._free_finished_seq_group(seq_group)
            if not seq_group.is_finished():
                remaining.append(seq_group)
        if len(self.free_requests["seq_ids"]) > 0 and get_role() == "SENDER":
            print(self.free_requests)
            print(self.free_requests)
            sr_obj(self.free_requests, 0, 1)
