...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            output_len = max(output_len, outputs[i].receive_times)
            ...
            if output_len > 1:
                tpot = (outputs[i].latency - outputs[i].ttft) / (output_len -
                                                                 1)
                tpots.append(tpot)
            # print(output_len, i, tpot, np.array(outputs[i].itl).mean())
...
async def benchmark(
    ...
    # we should filter out first 20% and last 20% of the outputs
    total_input = sum([input_request[1] for input_request in input_requests])
    input_requests = input_requests[int(len(input_requests) * 0.2):int(
        len(input_requests) * 0.8)]
    outputs = outputs[int(len(outputs) * 0.2):int(len(outputs) * 0.8)]
...
def main(args: argparse.Namespace):
    ...
    # wait until the url is online
    while True:
        ret = os.system("curl http://localhost:8000/v1/completions")
        if ret == 0:
            break
        time.sleep(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="sharegpt",
        choices=["sharegpt", "sonnet", "random", "hf","azure"],
